...
## Сборки и результат оптимизации

### Webpack (dev)
Сервер стартует стабильно, HMR работает, ошибок нет. Проект собирается ровно и предсказуемо.

### Webpack (prod)
После оптимизаций прод-сборка стала меньше:
- main.js с ~714 KiB → ~557 KiB
- часть кода вынесена в чанки
- сборка проходит чисто (только стандартные performance warning)

### Vite
Самая быстрая сборка:
- dev запускается за ~0.5 сек
- прод даёт лёгкие ассеты (~260–270 KiB gzip для основного бандла)
- без ошибок и лишнего шума

**Итог:** Webpack dev — стабильный, Webpack prod — похудел, Vite — самый быстрый вариант.
...
### До оптимизаций
Глубокое дерево рендера, много лишних перерисовок:
- форма логина перекрашивала почти всю страницу
- Header и Footer цеплялись за общий ререндер
- контекст тянул дерево вниз
- ProductList и MUI-компоненты перегружали граф

### После оптимизаций
Дерево стало аккуратнее и короче:
- Header/Footer не ререндерятся
- контекст стабилен
- список товаров вынесен в lazy-чанк
- рендерятся только нужные компоненты (ProductList)
- интерфейс стал отзывчивее

**Итог:** лишние ререндеры ушли, UI стал быстрее и стабильнее.
